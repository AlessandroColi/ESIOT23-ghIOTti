...
class ServoTimer2 
{
    public:
        ...
        void detach();
        void write(int); // store the pulse width in microseconds (between MIN_PULSE_WIDTH and MAX_PULSE_WIDTH) in the servo channel
        int read(); // returns current pulse width in microseconds for this servo (was read_us() in first release)
        boolean attached(); // return true if this servo is attached, otherwise false
